...
# Paths & constants
# ------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent.parent  # scratch/ 의 부모 = quant_invest/
...
def compute_from_stock_files(codes: list[str]) -> tuple[int | None, str]:
    ...
    Returns:
        (total_krw, note)
        - total_krw: 30개 종목의 5일 누적 외인 순매수 합계 (KRW)
        - note: 사람이 읽을 수 있는 설명
    """
...
def compute_from_heatmap(codes: list[str], data: dict) -> tuple[int | None, str]:
    ...
    # supply-signals의 foreign_5d lookup
    f5_map: dict[str, int] = {}
    for s in (data.get("buy") or []) + (data.get("sell") or []):
        c = s.get("code")
        if c:
...
def fetch_naver_kospi_foreign_today() -> float | None:
    ...
    """
    # 시도: sise_index.naver?code=KOSPI (외인/기관 일일 표 포함 가능성)
...
def main():
    ...
    if args.source in ("auto", "stocks"):
        ...
        if total is not None and (note.find("hit 0/miss") == -1) and total != 0 or (args.source == "stocks" and total is not None):
            # hit이 있거나 (0이 아닌 total), 또는 명시적 stocks 호출이면 사용
    ...
    if flow_5d_krw is None and args.source in ("auto", "signals"):
        # last-resort: supply-signals foreign_5d (shares) 합산 — KRW 환산 없음
        ...
        # shares만으로는 KRW 모름 → 0 처리 (실제 합리적 값 보장 불가)
        # 사용자에게 의미 있는 regime을 제공하기 위해 보수적으로 RED_EXTREME 표시
        flow_5d_krw = 0
    ...
    # 3) cross-check: Naver KOSPI 일일 외인
    ...
    # 9) stdout JSON (GH Actions / cron 로그용)
    print(json.dumps({
        "ok": True,
        "as_of": today_str,
        "regime": regime,
